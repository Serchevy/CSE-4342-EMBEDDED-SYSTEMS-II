...
void spi0write(uint8_t add, uint8_t data) {
    ...
    writeSpi0Data(0x40);        // Opcode = 0b0100 0000
    check = readSpi0Data();

    writeSpi0Data(add);         // Register Add
    check = readSpi0Data();

    writeSpi0Data(data);        // Data
    check = readSpi0Data();
...
uint8_t spi0read(uint8_t add) {
    ...
    setPinValue(SSI0FSS, 0);
    writeSpi0Data(0x41);        // Opcode = 0b0100 0001
    check = readSpi0Data();
